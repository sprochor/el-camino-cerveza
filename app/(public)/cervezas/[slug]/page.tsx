...
export default function CervezaDetailPage() {
      ...
      if (cervData) {
        ...
        if (errorRevs) {
          ...
          if (revsSimples) {
            ...
            if (session && revsSimples.some((r) => r.usuario_id === session.user.id))
              setYaReseno(true);
          }
        } else if (revs) {
          setResenas(revs);
          if (session && revs.some((r) => r.usuario_id === session.user.id))
            setYaReseno(true);
        }
      }
  ...
  const renderStars = (promedio: number) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <span
          key={i}
          className={`text-xl ${i <= Math.round(promedio) ? "text-amber-400" : "text-stone-600"}`}
        >
          ★
        </span>,
      );
  ...
  const promedioGeneral =
    resenas.length > 0
      ? (
          resenas.reduce(
            (acc, curr) =>
              acc + Number(curr.puntaje_general || curr.promedio_resena || 0),
            0,
          ) / resenas.length
        ).toFixed(1)
      : "S/P";
  ...
  const calcularPromedioCategoria = (categoria: string) => {
    ...
    return (
      resenas.reduce((acc, curr) => acc + Number(curr[categoria] || 0), 0) /
      resenas.length
    );
  };
  ...
  if (!cerveza)
    return (
      <div className="min-h-screen flex justify-center items-center bg-stone-50">
        <h1 className="text-2xl text-stone-800">Ups, se vació el barril.</h1>
      </div>
    );
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-4 mb-2">
              <div>
                ...
                {/* Enlace hacia la fábrica usando el SLUG de la cervecería */}
                <Link
                  href={`/cervecerias/${cerveza.cervecerias?.slug || cerveza.cerveceria_id}`}
                  className="text-lg text-gray-500 font-medium hover:text-amber-700 transition flex items-center justify-center md:justify-start gap-2"
                >
                  🏭 {cerveza.cervecerias?.nombre}{" "}
                  <span className="text-sm">
                    📍 {cerveza.cervecerias?.ciudad}
                  </span>
                </Link>
              </div>
              ...
              <div className="flex-[2] bg-stone-50 p-4 rounded-xl border border-gray-100 flex flex-col gap-3">
                {cerveza.presentaciones?.length > 0 && (
                  <div className="flex items-center gap-3">
                    <span className="text-gray-500 text-xs font-bold uppercase w-16">
                      Envase
                    </span>
                    <div className="flex flex-wrap gap-2">
                      {cerveza.presentaciones.map((p: string) => (
                        <span
                          key={p}
                          className="bg-white px-3 py-1 rounded shadow-sm text-sm font-bold text-stone-700 border border-gray-200"
                        >
                          {p}
                        </span>
                      ))}
              ...
              <div className="flex-1 flex items-center gap-4 bg-amber-50 p-4 rounded-xl border border-amber-200 text-stone-900">
                <div className="w-12 h-12 bg-white rounded-lg shadow-sm flex items-center justify-center p-1 shrink-0">
                  {vasoFinal.imagen_url ? (
                    <img
                      src={vasoFinal.imagen_url}
                      alt={vasoFinal.nombre}
        ...
        <div className="bg-stone-800 rounded-3xl p-6 md:p-10 shadow-sm text-white">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            {!usuario ? (
              <Link
                href="/login"
                className="bg-stone-700 hover:bg-stone-600 text-white font-bold px-4 py-2 rounded-xl transition text-sm"
              >
              ...
              <button
                ...
                className="bg-amber-500 hover:bg-amber-400 text-stone-900 font-black px-6 py-2 rounded-xl transition shadow-md w-full sm:w-auto"
              >
                Dejar mi reseña
              </button>
            )}
          ...
          {resenas.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-6">
              <div className="flex justify-between items-center border-b border-stone-700 pb-2">
                ...
                <div className="flex gap-1">
                  {renderStars(calcularPromedioCategoria("puntos_aroma"))}
                </div>
              </div>
              <div className="flex justify-between items-center border-b border-stone-700 pb-2">
                <span className="font-medium text-stone-300">Sabor</span>
                <div className="flex gap-1">
                  {renderStars(calcularPromedioCategoria("puntos_sabor"))}
                </div>
              </div>
              <div className="flex justify-between items-center border-b border-stone-700 pb-2">
                <span className="font-medium text-stone-300">Cuerpo</span>
                <div className="flex gap-1">
                  {renderStars(calcularPromedioCategoria("puntos_cuerpo"))}
                </div>
              </div>
              <div className="flex justify-between items-center border-b border-stone-700 pb-2">
                <span className="font-medium text-stone-300">Apariencia</span>
                <div className="flex gap-1">
                  {renderStars(calcularPromedioCategoria("puntos_apariencia"))}
                </div>
              </div>
            </div>
          ) : (
            <div className="text-center py-8 bg-stone-700/50 rounded-2xl border border-stone-600 border-dashed">
              <p className="text-stone-300 mb-2">
                Todavía nadie puntuó esta cerveza.
              </p>
              <p className="text-amber-400 font-bold">
                ¡Sé el primero en destaparla!
              </p>
            </div>
          )}
        </div>
        ...
        {resenas.length > 0 && (
          <div className="space-y-4 pt-4">
            <h3 className="text-2xl font-black text-stone-800 mb-6">
              Comentarios de la comunidad
            </h3>
            {resenas.map((resena) => (
              <div
                key={resena.id}
                className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row gap-4"
              >
                <div className="w-12 h-12 bg-stone-100 rounded-full flex items-center justify-center text-xl shrink-0 overflow-hidden border border-gray-200">
                  {resena.profiles?.avatar_url ? (
                    <img
                      src={resena.profiles.avatar_url}
                      alt="Avatar"
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                    <span className="font-bold text-stone-900">
                      {resena.profiles?.full_name || "Usuario Cervecero"}
                    </span>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <div className="flex text-amber-400 text-sm drop-shadow-sm">
                      {"★".repeat(
                        Math.round(
                          resena.puntaje_general || resena.promedio_resena || 5,
                        ),
                      )}
                      {"☆".repeat(
                        5 -
                          Math.round(
                            resena.puntaje_general ||
                              resena.promedio_resena ||
                              5,
                          ),
                      )}
                    </div>
